...
const suggestUsername = async (req, res) => {
  try {
    ...
    if (!username) {
      ...
      return apiErrorResponse(
        res,
        "Base name is required for username suggestions.",
        null,
        HTTP_STATUS.BAD_REQUEST
      );
    }
...
const becomeCurator = async (req, res) => {
  try {
    ...
    if (!socialId || !platform || !followers) {
      return apiErrorResponse(
        res,
        "Socail-Id, platforma and followers are required!"
      );
    }
    const socialMedia = await userService.becomeCurator(
      userId,
      platform,
      socialId,
      followers
...
const getFollowers = async (req, res) => {
  try {
    ...
    return apiSuccessResponse(
      res,
      "Fetched successfully followers",
      followers,
      HTTP_STATUS.OK
    );
  } catch (error) {
...
const setPassword = async (req, res) => {
  try {
    ...
    if (result.status) {
      apiSuccessResponse(
        res,
        "Password set successfully. Proceed to select user type.",
        result.data,
        HTTP_STATUS.OK
      );
    }
